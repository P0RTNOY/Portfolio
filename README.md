...
Course importing reads public page metadata when available. The admin can also paste course details from a provider page, such as the Udemy title, description, or "what you'll learn" text. Hugging Face uses the URL plus pasted details to suggest title, slug, descriptions, provider, and skill fields. If a provider such as Udemy blocks server-side metadata fetches with bot protection, the pasted text becomes the main context. Images and instructors stay blank when they are not actually available from metadata or pasted text. Course progress is private account data on platforms like Udemy, so progress, completion status, and certificate links remain admin-managed fields unless a future authenticated provider integration is added.
