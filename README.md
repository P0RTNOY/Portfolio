...
Course importing reads public page metadata when available. If a provider such as Udemy blocks server-side metadata fetches with bot protection, the importer falls back to URL-only Hugging Face suggestions so the admin can still apply conservative title, slug, description, provider, and skill fields. Images and instructors stay blank when they are not actually available from metadata. Course progress is private account data on platforms like Udemy, so progress, completion status, and certificate links remain admin-managed fields unless a future authenticated provider integration is added.
